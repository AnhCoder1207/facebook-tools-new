...
ffmpeg -i %1 -i "%SrcDir%\%rand%.mp3" -filter_complex amix=inputs=2:duration=shortest mix_audio/%1 -y
ffmpeg -i mix_audio/%1 -vf "[0:v]crop=in_w*0.86:in_h*0.86,scale=%width%:%height%,setpts=1/1.15*PTS" -af "[0:a]atempo=1.15" -preset ultrafast -crf 24 -shortest done/%1 -y
